...
if __name__ == '__main__':
    ...
    while time.time() < 1506491443:
        print(time.strftime('现在时间：%Y-%m-%d %H:%M:%S', time.localtime(time.time())),'请等待~(刷新时间为30秒，抢票的时候是每秒3次)')
        print('开始抢票时间：2017/9/27 13:50:43')
        time.sleep(30)
    while time.time() > 1506491443:
        print(time.strftime('开始抢票：%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
        while time.time() < 1506494452:
            maipiao_result = mai_piao(name=name, sellingPrice='300', activityId='599a76380cf2fa161b59c8d2',
                                      ticketId='76ee2d060dde40d5965302824c816518', limitBuyNum='2', sign=sign)
            i = i + 1
            # print(i)
            if i % 10 == 0:
                print('已经抢票次数:',i)
